...
export function NotificationsPage() {
  ...
  const tabs = [["all", "All"], ["unread", "Unread"], ["assigned", "Assigned"], ["comment", "Comments"], ["status", "Status Changes"], ["attachment", "Attachments"], ["sprint", "Sprint Updates"], ["due", "Due Date"]];
  const iconMap: Record<string, React.ReactNode> = {
    assigned: <Flag size={12} style={{ color: "#6d28d9" }} />, comment: <MessageSquare size={12} style={{ color: "#1d3f73" }} />,
    status: <CheckCircle size={12} style={{ color: "#2a8c3f" }} />, mention: <Hash size={12} style={{ color: "#e59f0c" }} />,
    review: <Eye size={12} style={{ color: "#5c6478" }} />, attachment: <Paperclip size={12} style={{ color: "#5c4a87" }} />,
    sprint: <Zap size={12} style={{ color: "#e59f0c" }} />, release: <Archive size={12} style={{ color: "#3d7a4e" }} />,
    due: <Calendar size={12} style={{ color: "#b91c1c" }} />,
  };
  const bgMap: Record<string, string> = { assigned: "#f5f3ff", comment: "#eef2ff", status: "#eef6f0", mention: "#fef5e4", review: "#f1f5f9", attachment: "#faf5ff", sprint: "#fef5e4", release: "#eef6f0", due: "#fef2f2" };
  const projectMap: Record<string, string> = { assigned: "Nexus Platform 2025", comment: "Nexus Platform 2025", status: "Nexus Platform 2025", mention: "Nexus Platform 2025", review: "Nexus Platform 2025", attachment: "Nexus Platform 2025", sprint: "Nexus Platform 2025", release: "Nexus Platform 2025", due: "Nexus Platform 2025" };
    ...
    <div className="flex-1 overflow-auto" style={{ backgroundColor: "#f0f2f5" }}>
      ...
      <div className="max-w-2xl mx-auto py-4 px-4 space-y-2">
        ...
        {filtered.map(n => {
          ...
          return (
            <div key={n.id} className="flex items-start gap-3 bg-white rounded p-3.5 cursor-pointer group" style={{ border: `1px solid ${n.read ? "#edf0f4" : "#bdd0ef"}`, backgroundColor: n.read ? "#fff" : "#f7f9fd", borderLeft: n.read ? "1px solid #edf0f4" : "3px solid #2558a6" }} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} onClick={() => setNotifs(prev => prev.map(x => x.id === n.id ? { ...x, read: true } : x))}>
              ...
              {hov && <button className="flex items-center gap-1 text-[11px] font-medium shrink-0 self-center px-2 py-1 rounded" style={{ color: "#2558a6" }} onClick={e => e.stopPropagation()}>Go to item <ChevronRight size={12} /></button>}
            </div>
          );
        })}
      </div>
