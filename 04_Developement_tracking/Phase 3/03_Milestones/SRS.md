...
-> User assigns Story/Defect, Feature or Epic items to the Milestone from their normal create/detail surfaces
-> Milestone Artifacts dashboard shows directly assigned items and the inherited descendant scope used by existing rollups
...
| P3-MS-FR-014 | Milestone Artifacts support directly assigned Story, Defect, Feature and Epic items. |
...
| P3-MS-FR-019 | Milestone Artifacts dashboard uses Backlog-compatible rows for Story/Defect and Portfolio-compatible identity/presentation for Feature/Epic. |
...
| P3-MS-FR-028 | **Future Backlog:** Milestone Artifacts provides `Add New Item`; US/DE reuse Backlog creation, Feature/Epic reuse Portfolio creation, and the current Milestone is prefilled. |
| P3-MS-FR-029 | Directly assigning a Feature includes its Story/Defect descendants in the Milestone's inherited artifact/rollup scope. Directly assigning an Epic includes its child Features and their Story/Defect descendants. |
| P3-MS-FR-030 | Direct and inherited artifact populations are de-duplicated by stable item ID. Removing a parent removes only relationships inherited from that parent; independently assigned descendants remain. |
| P3-MS-FR-031 | Existing Percent Done and Issues calculations remain unchanged. Only their eligible artifact population expands through Feature/Epic descendants; existing due-date-versus-Milestone-target issue detection remains in force. |
...
- Milestone Artifacts are assigned Story/Defect/Feature/Epic delivery items, not uploaded files or external links.
- Story/Defect rows reuse the Backlog presentation where possible; Feature/Epic rows reuse Portfolio identity and presentation.
- Feature/Epic descendant expansion is derived for display and existing Milestone rollups; it does not rewrite each descendant's direct Milestone or Release field.
- **Future Backlog:** `Add New Item` supports User Story, Defect, Feature and Epic. It reuses the normal Backlog/Portfolio create screens with current Milestone prefilled; `Create` returns to Artifacts and `Create with details` opens the same new item.
- A Story/Defect can be assigned to Milestone independently from Release assignment.
- Milestone artifact assignment is many-to-many: a Milestone can have many supported artifacts, and the same artifact may appear in more than one Milestone if scope rules allow it.
- Removing a directly assigned artifact from a Milestone removes that direct relationship and descendants inherited only through it; another direct assignment remains.
...
type MilestoneArtifactListResponse = {
  items: Array<BacklogWorkItemListItemDto | PortfolioItemListItemDto>;
  page: number;
  pageSize: number;
  total: number;
};
...
- Story/Defect rows reuse the Backlog list DTO; Feature/Epic rows reuse the Portfolio list DTO.
- Valid direct artifact types are Story, Defect, Feature and Epic. Task is excluded.
- The response includes directly assigned artifacts and de-duplicated inherited descendants of directly assigned Feature/Epic records.
- Inline edit follows the source Backlog or Portfolio editability rules for each item type.
...
{
  "artifactIds": ["US-4821", "DE-1142", "FE-318", "EP-101"]
}
```

Rules:

- The payload replaces the directly assigned Milestone artifact list; inherited descendants are derived and are not written into this list.
- Each artifact must be accessible to the current user.
- Each artifact must belong to an allowed Project/Team scope for the Milestone.
...
| P3-MS-Q07 | What exact artifact behavior does Milestone need? | Confirmed: direct US/DE/Feature/Epic; Feature/Epic descendants are inherited for display/rollup and de-duplicated |
...
| P3-MS-Q13 | Is Add New Item required now? | No. Deferred to Future Backlog on 2026-08-15; current assignment comes from Work Item and Portfolio Item surfaces |
...
8. Milestone Artifacts tab lists directly assigned Story/Defect/Feature/Epic records and the inherited descendant population using source-compatible presentation.
...
17. Feature/Epic descendant expansion preserves existing Milestone rollup formulas and de-duplicates every item by stable ID.
18. **Future Backlog:** Add New Item creates exactly one US/DE/Feature/Epic with the current Milestone prefilled and opens the corresponding Work Item or Portfolio Item detail when Create with details is used.

## 8. Test Scenarios

| ID | Scenario | Expected |
|---|---|---|
| P3-MS-TS-001 | Assign US/DE/Feature/Epic to Milestone from its normal create/detail surface | Same directly assigned item appears in that Milestone's Artifacts list |
| P3-MS-TS-002 | Assign Feature/Epic to Milestone | Descendants enter inherited artifact/rollup scope once, with no duplicate counting |
| P3-MS-TS-FB-001 | Future Backlog: Milestone Artifacts > Add New Item | Shared US/DE/Feature/Epic create flow opens with current Milestone prefilled |
| P3-MS-TS-FB-002 | Future Backlog: Create with details from Milestone | Exactly one item is created and the matching Work Item or Portfolio Item detail opens |
